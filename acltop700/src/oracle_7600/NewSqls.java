...
public interface NewSqls {
    ...
    String new_update_result="update l_testresult set testresult=?,measuretime=? ,resultflag=?,correctflag=?,deviceid=?,refhi=?,reflo=?,method=? where sampleno=? and testid=?";
    String getSampleByCode="select age, sex, sampleno, cycle,ageunit,sampletype,ylxh from L_SAMPLE where barcode=? and rownum<=1";
    String getSampleByNO="select age, sex, sampleno, cycle,ageunit,sampletype,ylxh from L_SAMPLE where sampleno=? and rownum<=1";
