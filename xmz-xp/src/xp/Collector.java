...
//消息的生成和分发   acltop700的collect   收到EOT，回复ACK
public class Collector implements IBuilder {
	...
	public void receivePrint(String text) {
        System.out.println(CommonUtil.getCurrentDateTime(new Date())+" Received---"+text);
	}

	public void sendPrint(String text) {
		System.out.println(CommonUtil.getCurrentDateTime(new Date())+" Send-------"+text);
	}
	...
	protected void responeMonitor(String str) {
		...
		lastSend = str;
		/*isResponed = false;
		responseTimer = new Thread(responseTimerRun); // 等待响应
		responseTimer.start();*/
	}
